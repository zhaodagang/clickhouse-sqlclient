...
export default class QueryLaunch extends Component<Props> {
  ...
  getQueryDetailsDescription = (queryData) => {
    ...
    return `返回 ${queryData.rows} 行, 
    消耗 ${queryData.statistics.elapsed.toFixed(3)}ms, 
    ${queryData.statistics.rows_read} 行 
    处理 ${prettyBytes(parseInt(queryData.statistics.bytes_read, 10))} 的数据`;
  };
  ...
  useDatabase = (e) => {
    ...
    toaster.show({
      message: e.target.value ? `使用数据库 ${e.target.value}.` : '使用数据库 default',
      intent: Intent.WARNING,
      icon: 'database',
      timeout: 5000
    });
      ...
      <div id="editor" style={{ height: '100%' }}>
        ...
        >
          ...
          <NavbarGroup align={Alignment.LEFT} style={{ height: '35px' }}>
            ...
            <Tooltip content="运行查询" position={Position.BOTTOM}>
              <AnchorButton
            ...
            <Tooltip content="忽略查询" position={Position.BOTTOM}>
              <AnchorButton
            ...
            <div className="pt-select pt-dark pt-minimal database-select">
              <select id="select" onChange={this.useDatabase} >
                {
                  !localStorage.getItem(localStorageVariables.database.use) ? <option value="">选择数据库</option> : null
                }
          ...
          <NavbarGroup align={Alignment.RIGHT} style={{ height: '35px' }}>
            ...
            <Tooltip content="查询历史" position={Position.LEFT}>
              <Button
            ...
            <Tooltip content="快捷键和帮助" position={Position.LEFT}>
              <Button
        ...
        >
          <div className="pt-dialog-body">
            ...
            <h4>帮助</h4>
            选择要运行查询本地化的文本或在编辑器中执行所有文本.

            <br /><br /><br />

            <h4>快捷键</h4>
            <b>CTRL or COMMAND + ENTER</b> -- 运行查询
